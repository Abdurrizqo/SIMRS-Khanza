...
public class koneksiDB {
            ...
            try {
                if (connection.isClosed()) {
                    ...
                    dataSource.setURL("jdbc:mysql://" + EnkripsiAES.decrypt(prop.getProperty("HOST")) + ":"
                            + EnkripsiAES.decrypt(prop.getProperty("PORT")) + "/"
                            + EnkripsiAES.decrypt(prop.getProperty("DATABASE"))
                            + "?zeroDateTimeBehavior=convertToNull&autoReconnect=true&cachePrepStmts=true");
                    dataSource.setUser(EnkripsiAES.decrypt(prop.getProperty("USER")));
                    dataSource.setPassword(EnkripsiAES.decrypt(prop.getProperty("PAS")));
                    connection = dataSource.getConnection();
                }
    ...
    private static void closeApplication(Exception e) {
        JOptionPane.showMessageDialog(null, "Jaringan Bermasalah, Aplikasi Akan Di Keluarkan");
        System.exit(1); // Menghentikan aplikasi dengan status kode 1 (error)
    }
